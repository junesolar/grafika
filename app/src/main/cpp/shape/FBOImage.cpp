...
FBOImage::FBOImage() {
    ...
    auto imageData = service::GetService<IFileLoader>()->loadFile("image_test.jpg");
...
bool FBOImage::CreateFrameBufferObj() {
    // 创建并初始化 FBO 纹理
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 404, 336, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
...
void FBOImage::draw(int width, int height) {
    LOGCATE("FBOImage::draw width=%d, height=%d", width, height);

    // 离屏渲染
    glPixelStorei(GL_UNPACK_ALIGNMENT,1);
    glViewport(0, 0, 404, 336);
